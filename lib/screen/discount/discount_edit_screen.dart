...
class _DiscountEditScreenState extends State<DiscountEditScreen> {
  ...
  void initState() {
    ...
    _nameCtrl.text = widget.discount.discountName;
    _priceCtrl.text = widget.discount.discountPrice.toString();
    _requiredPointsCtrl.text = widget.discount.requiredPoints.toString();
    _selectedCategory = widget.discount.discountCategory;
    _dateStartCtrl.text = widget.discount.dateStart;
    _dateEndCtrl.text = widget.discount.dateEnd;
    _discountStatus = widget.discount.discountStatus;
  }
  ...
  Future<void> _submitUpdate() async {
    ...
    try {
      ...
      if (token == null) {
        _showSnack("⚠️ Phiên đăng nhập hết hạn, vui lòng đăng nhập lại.");
        return;
      }

      final id = widget.discount.discountId.trim();
      if (id.isEmpty) {
        _showSnack("Không tìm thấy mã giảm giá (ID).");
        return;
      }
      ...
      if (res.statusCode == 200 || res.statusCode == 204) {
        ...
        _showSnack("❌ Lỗi cập nhật (${res.statusCode})");
      }
    } catch (e) {
      _showSnack("Lỗi: $e");
    } finally {
                    ...
                    child: Column(
                      children: [
                        ...
                        TextFormField(
                          ...
                          validator: (v) =>
                          v!.isEmpty ? "Vui lòng nhập giá trị giảm" : null,
                        ),
                        const SizedBox(height: 18),
                        ...
                        TextFormField(
                          ...
                          validator: (v) =>
                          v!.isEmpty ? "Vui lòng nhập điểm cần đổi" : null,
                        ),
                        const SizedBox(height: 18),
                        ...
                        // 🔹 Trạng thái
                        SwitchListTile(
                          title: const Text("Trạng thái áp dụng"),
                          subtitle:
                          const Text("Bật để mã giảm giá có hiệu lực"),
                          value: _discountStatus,
                          activeColor: mainColor,
                          onChanged: (val) =>
                              setState(() => _discountStatus = val),
                        ),
                        const SizedBox(height: 30),
