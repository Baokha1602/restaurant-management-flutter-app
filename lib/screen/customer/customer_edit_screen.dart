...
class _CustomerEditScreenState extends State<CustomerEditScreen> {
  ...
  void initState() {
    ...
    _pointCtrl = TextEditingController(text: widget.customer.point?.toString() ?? "0");
    _dobCtrl = TextEditingController(
      text: widget.customer.dateOfBirth?.split('T').first ?? '',
    );
  ...
  Future<void> _pickDate() async {
    ...
    if (picked != null) setState(() => _dobCtrl.text = picked.toIso8601String().split('T')[0]);
  }

  void _updateRankByPoint() {
    final point = int.tryParse(_pointCtrl.text) ?? 0;
    if (widget.ranks.isEmpty) return;
    widget.ranks.sort((a, b) => (a.rankPoint).compareTo(b.rankPoint));
    final rank = widget.ranks.lastWhere(
          (r) => point >= (r.rankPoint ?? 0),
      orElse: () => widget.ranks.first,
    );
    widget.customer.rankId = rank.rankId;
    widget.customer.rankName = rank.rankName;
  }
  ...
  Future<void> _saveCustomer() async {
    ...
    try {
      widget.customer
        ...
        ..point = int.tryParse(_pointCtrl.text) ?? 0
        ..gender = _gender
                    ...
                    child: Column(
                      children: [
                        ...
                        TextFormField(
                          controller: _dobCtrl,
                          readOnly: true,
                          decoration: _inputStyle("Ngày sinh", Icons.calendar_today)
                              .copyWith(
                        ...
                        TextFormField(
                          ...
                          decoration: _inputStyle("Điểm tích luỹ", Icons.star),
                          onChanged: (_) => _updateRankByPoint(),
                        ),
